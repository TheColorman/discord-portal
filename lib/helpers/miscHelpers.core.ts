...
export default class BaseHelpersCore {
  ...
  public async enqueueMessageEvent(messageEvent: MessageEvent) {
    ...
    const messageEventQueue = messageEvent.queue() || new Queue<MessageEvent>();
  ...
  public async convertStickers(stickers: Collection<string, Sticker>) {
    const convertedPromise: Promise<string | null>[] = stickers.map(
      async (s) => {
        ...
        if (!stickerFile) return new Promise((resolve) => resolve(s.url));
        return stickerFile;
      },
    );
    ...
    return (converted.filter((sticker) => sticker !== null) as string[]).map(
      (s) => new AttachmentBuilder(s),
    );
  }
