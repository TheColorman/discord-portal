...
export default class DiscordHelpersCore extends DatabaseHelpersCore {
  ...
  constructor(stateDirectory: string, client: Client, db: Database) {
    super(stateDirectory, db);
    this.client = client;
  }
  ...
  public async deletePortalConnection(
    ...
    this.run("DELETE FROM portalConnections WHERE channelId = ?", [channelId]);
  ...
  public async createWebhook(channel: ValidChannel): Promise<Webhook> {
    const webhook = await channel.createWebhook({
      ...
      avatar: webhookAvatars[Math.floor(Math.random() * webhookAvatars.length)],
  ...
  public async sendMessageToPortalAsBot({
    ...
    const promises = portalConnections.map(async (portalConnection) => {
      const channel = await this.safeFetchChannel(portalConnection.channelId);
      if (!channel) {
  ...
  public async sendMessageToPortalAsWebhook({
    ...
    const promises = portalConnections.map(async (portalConnection) => {
      ...
      const localPortalReference = (() => {
        ...
        const portalReferences = this.getPortalMessages(portalReferenceId);
  ...
  public async formatWebhookReply({
    ...
    const prepend =
      (await (async () => {
        ...
        const channel = await this.safeFetchChannel(portalMessage.channelId);
        ...
        message.content = message.content.replace(/h(ttps?:\/\/)/gm, "ḩ$1");
  ...
  public async editMessageAsBot({
    ...
    const promises = portalMessages.map(async (portalMessage) => {
      ...
      const messageEditOptions =
        typeof options === "function" ? await options(message) : options;
  ...
  public async updatePortalMessageAsWebhook({
    ...
    options:
      ...
      | ((
          message: Message,
        ) => Promise<WebhookMessageEditOptions> | WebhookMessageEditOptions);
  }) {
  ...
  public async updatePortalMessage({
    ...
    const promises = oldAttachments.map(async (pm) => {
      if (!pm.attachmentId || newAttachments.includes(pm.attachmentId)) return;
